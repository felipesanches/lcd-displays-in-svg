...
function print_text(text_string, position){
    for (var i=0; i<text_string.length; i++){ 
        setDigit(11-(position+i), text_string[text_string.length - 1 - i]);
    }
}
...
function print_number(value, position, field_length){
    for (var digit=position; digit<=position+field_length-1; digit++){
        if (digit>=0 && digit<=11){
            if (value){
                setDigit(11-digit, String.fromCharCode(0x30 + value%10));
...
function scroll_text(text_string, position){
    var i = timer % (text_string.length + 12);
    text_string = "            " + text_string + "            ";
    print_text(text_string.slice(i, i+12), position);
}
...
function init_car_radio(){
    reset_display();
    setInterval(clock_tick, 300);
}
